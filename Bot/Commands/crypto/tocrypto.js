...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('convert')
        .setDescription("Convert an amount of usd into its crypto coin equivilant")
        ...
        .addNumberOption(option => option
            .setName("amount")
            .setDescription("How much to convert (defaults to 100 if not provided)")
        ),
    async execute(interaction) {
        ...
        const amount = await interaction.options.getNumber('amount') ?? 100
        let coin = input
        ...
        await interaction.reply({
            embeds: [embed.setColor("Blue").setDescription(`${amount / result.data.market_data.current_price.usd} ${coin}`).setTitle(`${amount}$ translates into:`)]
        })
    }
}
